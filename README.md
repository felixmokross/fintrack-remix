...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create fintrack
  fly apps create fintrack-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app fintrack
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app fintrack-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'fintrack-staging' does not match app name in config file 'fintrack'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name fintrack-db
  fly postgres attach --postgres-app fintrack-db --app fintrack

  fly postgres create --name fintrack-staging-db
  fly postgres attach --postgres-app fintrack-staging-db --app fintrack-staging
  ```
