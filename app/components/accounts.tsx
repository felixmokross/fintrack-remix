...
export function EditAccountModal({
  ...
  const [preExisting, setPreExisting] = useState(account.preExisting);

  const disabled = action.state !== "idle";
  return (
    <Modal open={open} onClose={onClose} size={ModalSize.LARGE}>
      <action.Form method="post" action={`${account.id}/edit`}>
        <fieldset disabled={disabled}>
          <Modal.Body title="Edit Account" icon={PencilIcon}>
            <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
              ...
              <AccountTypeRadioGroup
                ...
                error={action.data?.errors?.type}
                groupClassName="sm:col-span-3"
                defaultValue={action.data?.values?.type || account.type}
                onChange={setType}
                disabled={disabled}
              />
              {type === AccountType.ASSET && (
              ...
              <AccountUnitRadioGroup
                ...
                error={action.data?.errors?.unit}
                groupClassName="sm:col-span-3 sm:col-start-1"
                defaultValue={action.data?.values?.unit || account.unit}
                onChange={setUnit}
                disabled={disabled}
              />
              {unit === AccountUnit.CURRENCY && (
              ...
              {unit === AccountUnit.STOCK && (
                <Select
                  ...
                  groupClassName="sm:col-span-3"
                  defaultValue={
                    action.data?.values?.stockId || account.stockId || undefined
                  }
                >
                  <option value=""></option>
                  {stocks.map((stock) => (
...
type EditAccountModalProps = {
  open: boolean;
  data: SerializeType<EditAccountLoaderData>;
  onClose: () => void;
};
